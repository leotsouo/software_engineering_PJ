...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        // 重導並帶 success
        header('Location: edit_student.php?id=' . urlencode($student_id) . '&success=1');
        exit;
    }
}

// 3. 讀取原始資料
$sql = "SELECT Name, Phone FROM teammember WHERE StudentID = :id";
$stmt = $pdo->prepare($sql);
...
<head>
  <meta charset="UTF-8">
  <title>編輯學生 <?= htmlspecialchars($student_id) ?></title>
...
<body>
  <div class="container">
    <h1>編輯學生 <?= htmlspecialchars($student_id) ?></h1>
    ...
    <form method="post" action="edit_student.php?id=<?= urlencode($student_id) ?>">
