...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        header('Location: edit_teacher.php?id=' . urlencode($teacher_id) . '&success=1');
        exit;
    }
}

// 3. 讀取原始資料
$sql = "SELECT Name, Password FROM teacher WHERE TeacherID = :id";
$stmt = $pdo->prepare($sql);
...
<head>
  <meta charset="UTF-8">
  <title>編輯教師 <?= htmlspecialchars($teacher_id) ?></title>
...
<body>
  <div class="container">
    <h1>編輯教師 <?= htmlspecialchars($teacher_id) ?></h1>
    ...
    <form method="post" action="edit_teacher.php?id=<?= urlencode($teacher_id) ?>">
